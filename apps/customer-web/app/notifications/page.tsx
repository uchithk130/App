        ...
        <div className="min-h-dvh bg-[#f7f8f7] pb-28 lg:pb-12">
          <header className="sticky top-0 z-30 border-b border-slate-100 bg-white/95 backdrop-blur-md">
            <div className="flex items-center gap-3 px-4 py-3">
              ...
              {unread > 0 && (
                <button type="button" onClick={() => markAllRead.mutate()} disabled={markAllRead.isPending} className="flex items-center gap-1 rounded-full bg-emerald-50 px-3 py-1.5 text-[11px] font-bold text-emerald-600 transition hover:bg-emerald-100 disabled:opacity-50">
                  <CheckCheck className="h-3.5 w-3.5" /> Mark all read
                </button>
              )}
            </div>
          </header>
          <main className="mx-auto max-w-2xl px-4 pt-4 lg:px-6">
            {notifs.isLoading ? (
              ...
              <div className="flex flex-col items-center pt-20 text-center">
                ...
                <p className="mt-1 text-xs text-slate-400">Order updates will appear here.</p>
              </div>
            ) : (
              <div className="space-y-2">
                {items.map((n) => (
                  <button
                    ...
                    className={`flex w-full items-start gap-3 rounded-xl p-3.5 text-left transition ${n.readAt ? "bg-white" : "bg-emerald-50/50 ring-1 ring-emerald-100"}`}
                  >
                    <div className={`mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-full ${n.readAt ? "bg-slate-100 text-slate-400" : "bg-emerald-100 text-emerald-600"}`}>
                    ...
                    {!n.readAt && <span className="mt-1 h-2 w-2 shrink-0 rounded-full bg-emerald-500" />}
                  </button>
                ))}
              </div>
            )}
